...
def check_print(env,p1,p2):
    ...
    Parameter p2: The result for player 1
...
def grade_player1(file,step=0,outp=sys.stdout):
    ...
    if error == PRINT_WRONG_LINES:
        outp.write("Function roll_off has more than one print statement.\n")
        score -= 0.2
        if not step:
            return (FAIL_INCORRECT, max(0,score))
    ...
    if error == PRINT_WRONG_LINES:
        outp.write("Function roll_off has more than one print statement [2nd Attempt].\n")
        score -= 0.2
        if not step:
            return (FAIL_INCORRECT, max(0,score))
    elif error == PRINT_NO_PERIOD:
    ...
    if error == PRINT_WRONG_LINES:
        outp.write("Function roll_off has more than one print statement [3rd Attempt].\n")
        score -= 0.2
        if not step:
            return (FAIL_INCORRECT, max(0,score))
    elif error == PRINT_NO_PERIOD:
